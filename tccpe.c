...
static int pe_fwrite(void *data, unsigned len, /*FILE *fp*/ int fp_p, DWORD *psum)
{
    if (psum) {
        DWORD sum = *psum;
        WORD *p = data;
    ...
    return len == write(fp_p, data, 1*len) ? 0 : -1;
}

static void pe_fpad(/*FILE *fp*/ int fp_p, DWORD new_pos)
{
    DWORD pos = lseek(fp_p, 0, SEEK_CUR);
    while (++pos <= new_pos)
        dputc(0, fp_p);
}
...
#define CHARACTERISTICS_DLL 0x230F
    ...
    op_p = open(pe->filename, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666);
    if (-1 == op_p) {
    ...
    sum = 0;
    pe_fwrite(&pe_header, sizeof pe_header, op_p, &sum);
    for (i = 0; i < pe->sec_count; ++i)
        pe_fwrite(&pe->sec_info[i].ish, sizeof(IMAGE_SECTION_HEADER), op_p, &sum);
    pe_fpad(op_p, pe->sizeofheaders);
    for (i = 0; i < pe->sec_count; ++i) {
        si = pe->sec_info + i;
        psh = &si->ish;
        if (si->data_size) {
            pe_fwrite(si->data, si->data_size, op_p, &sum);
            file_offset = psh->PointerToRawData + psh->SizeOfRawData;
            pe_fpad(op_p, file_offset);
        }
    }

    pe_header.opthdr.CheckSum = sum + file_offset;
    lseek(op_p, offsetof(struct pe_header, opthdr.CheckSum), SEEK_SET);
    pe_fwrite(&pe_header.opthdr.CheckSum, sizeof pe_header.opthdr.CheckSum, op_p, NULL);
    close (op_p);
#ifndef _WIN32
...
static void pe_build_exports(struct pe_info *pe)
{
    ...
    sym_count = 0, sorted = NULL, op_p = -1;
    ...
    op_p = open(buf,  O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666);
    if (-1 == op_p) {
        tcc_error_noabort("could not create '%s': %s", buf, strerror(errno));
    } else {
        dprintf(op_p, "LIBRARY %s\n\nEXPORTS\n", dllname);
        if (pe->s1->verbose)
...
static void pe_print_section(/*FILE * f*/ int f_p, Section * s)
{

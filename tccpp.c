...
static void tok_print(const char *msg, const int *str)
{
    ...
    fp_p = tcc_state->ppfp_p;
    dprintf(fp_p, "%s", msg);
    while (str) {
	TOK_GET(&t, &str, &cval);
	if (!t)
	    break;
	dprintf(fp_p, " %s" + s, get_tok_str(t, &cval)), s = 1;
    }
    dprintf(fp_p, "\n");
}
...
static void pp_line(TCCState *s1, BufferedFile *f, int level)
{
	...
	dprintf(s1->ppfp_p, "#line %d \"%s\"\n", f->line_num, f->filename);
    } else {
	dprintf(s1->ppfp_p, "# %d \"%s\"%s\n", f->line_num, f->filename,
	    level > 0 ? " 1" : level < 0 ? " 2" : "");
    }
...
static void define_print(TCCState *s1, int v)
{
    ...
    fp_p = s1->ppfp_p;
    dprintf(fp_p, "#define %s", get_tok_str(v, NULL));
    if (s->type.t == MACRO_FUNC) {
        Sym *a = s->next;
        dprintf(fp_p,"(");
        if (a)
            for (;;) {
                dprintf(fp_p,"%s", get_tok_str(a->v & ~SYM_FIELD, NULL));
                if (!(a = a->next))
                    break;
                dprintf(fp_p,",");
            }
...
static void pp_debug_defines(TCCState *s1)
{
    ...
    if (t == TOK_DEFINE) {
        ...
        dprintf(fp_p, "#undef %s\n", vs);
    } else if (t == TOK_push_macro) {
        dprintf(fp_p, "#pragma push_macro(\"%s\")\n", vs);
    } else if (t == TOK_pop_macro) {
        dprintf(fp_p, "#pragma pop_macro(\"%s\")\n", vs);
    }
    pp_debug_tok = 0;
}
...
ST_FUNC int tcc_preprocess(TCCState *s1)
{
    ...
    for (;;) {
        ...
        white[spcs] = 0, dputs(white, s1->ppfp_p), spcs = 0;
        dputs(p = get_tok_str(tok, &tokc), s1->ppfp_p);

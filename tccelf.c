...
static void tcc_output_binary(TCCState *s1, /*FILE *f*/ int f_p,
                              const int *sec_order)
{
    Section *s;
    int i, offset, size;
...
static void tcc_output_elf(TCCState *s1, /*FILE *f*/ int f_p, int phnum, ElfW(Phdr) *phdr,
                           int file_offset, int *sec_order)
{
    ...
    write(f_p, &ehdr, 1*sizeof(ElfW(Ehdr)));
    write(f_p, phdr, 1*phnum * sizeof(ElfW(Phdr)));
    ...
    for(i = 0; i < s1->nb_sections; i++) {
        ...
        write(f_p, sh, 1*sizeof(ElfW(Shdr)));
    }
}
                              ...
                              ElfW(Phdr) *phdr, int file_offset, int *sec_order)
{
    ...
    if (s1->output_format == TCC_OUTPUT_FORMAT_COFF)
        tcc_output_coff(s1, f_p);
    else
#endif
    if (s1->output_format == TCC_OUTPUT_FORMAT_ELF)
        tcc_output_elf(s1, f_p, phnum, phdr, file_offset, sec_order);
    else
        tcc_output_binary(s1, f_p, sec_order);
    close(f_p);
